...
(defmethod render ((page wiki-viewer))
  ...
  (when (ucw-core::was-called-p page)
    (<:p (<ucw:a :action (answer) "Previous Page"))))

;;;; *** The view-page action

(defmethod/cc view-page ((self wiki-viewer) page-name)
  (unless (find-wiki-page page-name)
    (edit-page self page-name))
  (call 'wiki-viewer :page-name page-name))
...
(defmethod/cc edit-page ((self wiki-viewer) page-name)
...
(defmethod/cc save-changes ((self wiki-editor) name summary contents)
  (update-wiki-page (page-name self)
                    (make-instance 'wiki-edit
                                   :author name
                                   :summary summary
                                   :contents contents))
  (answer (page-name self)))
  ...
  (let ((name "")
    ...
    (<ucw:form :action (save-changes w name summary contents)
      ...
      (<:p
        ...
        (<ucw:input :accessor name :id "author"))
      (<:p
        (<:label :for "summary" "Summary:")
        (<ucw:input :accessor summary :id "summary"))
      (<:p (<:input :type "submit" :value "Save Changes")
	   (<ucw:submit :action (answer) :value "Cancel")))))
...
(defentry-point "edit.ucw" (:application *example-application*)
  ...
  (cond
    ((not (get-session-value 'user))
     (call 'ucw::redirect-component :target "wiki-login.ucw"))
    (contents
...
(defentry-point "wiki-login.ucw" (:application *example-application*)
  ...
  (if (and (string= "wiki" username) (string= "wiki" password))
      (progn
        ...
        (call 'ucw::redirect-component :target "view.ucw"))
...
(defmethod render ((login wiki-login))
  (let ((username "")
    ...
    (<ucw:form :action (wiki-login login username password)
      ...
      (<:p
        (<:label :for "username" "Username: ")
        (<ucw:input :accessor username :id "username"))
      (<:p
        (<:label :for "password" "Password: ")
        (<ucw:input :type "password" :accessor password :id "password"))
      (<:p
        (<ucw:submit :action (wiki-login login username password))))))

(defmethod/cc wiki-login ((self wiki-login) username password)
